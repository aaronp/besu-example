...
# Backup each member node
for member in "${members[@]}"; do
    member_num=${member#member}
    echo "Backing up $member..."
    "$SCRIPT_DIR/backup-member1.sh" "$member_num"
    if [ $? -ne 0 ]; then
        echo "Error: Failed to backup $member"
        exit 1
    fi
done

echo "All member nodes have been backed up successfully!" 